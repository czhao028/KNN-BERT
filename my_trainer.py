...
class Trainer:
    ...
    def __init__(
        ...
        #
        # cos_mat, w2i, i2w = self.get_sim_embed(args.sim_embed, args.cosine_npy)
        # self.cos_mat, self.w2i, self.i2w = cos_mat, w2i, i2w
        self.train_inputs, self.train_feature, self.train_labels = None, None, None

        self.best_metric = None
        # if not self.args.use_original_model:
        #     pass
        # self.update_train_data_feature()
    ...
    def tokenizer_to_tensor(self, sample: List[str]):
        result = self.tokenizer(sample, padding="max_length", max_length=self.args.max_length, truncation=True)
    ...
    def predict_queue_knn_test(self,
        ...
        feature_queue = model.feature_queue.clone().detach()
        label_queue = model.label_queue.clone().detach()

        eval_dataloader = dataloader
        num_examples = self.num_examples(eval_dataloader)

        logger.info("***** Running %s *****", "predict")
        logger.info("  Num examples = %d", num_examples)

        labels_host: Union[torch.Tensor, List[torch.Tensor]] = None
        preds_host_knn: Union[torch.Tensor, List[torch.Tensor]] = None
        preds_host_cls: Union[torch.Tensor, List[torch.Tensor]] = None
        preds_host_combine: Union[torch.Tensor, List[torch.Tensor]] = None
        ...
        preds_gatherer_knn = DistributedTensorGatherer(world_size, num_examples)
        preds_gatherer_cls = DistributedTensorGatherer(world_size, num_examples)
        preds_gatherer_combine = DistributedTensorGatherer(world_size, num_examples)
        ...
        for step, inputs in enumerate(eval_dataloader):
            ...
            model_output, logits_cls = model.predict(inputs)

            model_output = l2norm(model_output)
            cos_sim = torch.einsum("nk, mk -> nm", [model_output, feature_queue])
            top_k = self.args.top_k
            _, index = cos_sim.topk(k=top_k, dim=-1)
            train_labels = label_queue.repeat([int(batch_size), 1])
            dev_labels = train_labels.gather(-1, index)

            dev_labels = dev_labels.unsqueeze(dim=-1)
            dev_labels = torch.zeros([batch_size, top_k, self.args.num_labels]).to("cuda").scatter_(-1, dev_labels, 1)
            dev_labels = dev_labels.sum(dim=1)
            dev_soft = dev_labels / top_k
            logits_soft = logits_cls.softmax(dim=-1)
            combine_logits = (dev_soft * self.args.contrastive_rate_in_inference +
                              (1 - self.args.contrastive_rate_in_inference) * logits_soft)

            preds_host_knn = dev_soft if preds_host_knn is None else nested_concat(preds_host_knn, dev_soft,
                                                                                   padding_index=-100)
            preds_host_cls = logits_cls if preds_host_cls is None else nested_concat(preds_host_cls, logits_cls,
                                                                                     padding_index=-100)
            preds_host_combine = combine_logits if preds_host_combine is None else nested_concat(preds_host_combine,
                                                                                                 combine_logits,
                                                                                                 padding_index=-100)

            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                preds_gatherer_knn.add_arrays(self._gather_and_numpify(preds_host_knn, "eval_preds_knn"))
                preds_gatherer_cls.add_arrays(self._gather_and_numpify(preds_host_cls, "eval_preds_cls"))
                preds_gatherer_combine.add_arrays(self._gather_and_numpify(preds_host_combine, "eval_preds_combine"))

                # Set back to None to begin a new accumulation
                preds_host_knn, preds_host_cls, preds_host_combine = None, None, None

        preds_gatherer_knn.add_arrays(self._gather_and_numpify(preds_host_knn, "eval_preds_knn"))
        preds_gatherer_cls.add_arrays(self._gather_and_numpify(preds_host_cls, "eval_preds_cls"))
        preds_gatherer_combine.add_arrays(self._gather_and_numpify(preds_host_combine, "eval_preds_combine"))
        preds_knn = preds_gatherer_knn.finalize()
        preds_cls = preds_gatherer_cls.finalize()
        preds_combine = preds_gatherer_combine.finalize()

        metrics = {}

        return PredictionOutput(predictions=preds_knn, label_ids=None, metrics=metrics)

    def predict_queue_knn(self):
        model = self.model
        model.eval()
        ...
        for step, inputs in enumerate(eval_dataloader):
            ...
            preds_host_combine = combine_logits if preds_host_combine is None else nested_concat(preds_host_combine,
                                                                                                 combine_logits,
                                                                                                 padding_index=-100)
            labels_host = label if labels_host is None else nested_concat(labels_host, label, padding_index=-100)
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                ...
                # Set back to None to begin a new accumulation
                preds_host_knn, preds_host_cls, preds_host_combine, labels_host = None, None, None, None
        ...
        metrics = self.compute_metrics(
            MyEvalPrediction(prediction_by_knn=preds_knn, prediction_by_cls=preds_cls, prediction_combine=preds_combine,
                             label_ids=label_ids))
    ...
    def generate_single_sample(self, label: torch.Tensor):
        ...
        for index in range(label.shape[0]):
            input_label = int(label[index])
            positive_sample.append(random.choice(self.negative_data[input_label]))

        return self.list_item_to_tensor(positive_sample)

    def generate_negative_sample(self, label: torch.Tensor):
        negative_sample = []
        for index in range(label.shape[0]):
            input_label = int(label[index])
            negative_labels = self.negative_keys.copy()
            negative_labels.remove(input_label)
            for _ in range(self.negative_num):
                negative_label = random.choice(negative_labels)
                negative_sample.append(random.choice(self.negative_data[negative_label]))

        return self.reshape_dict(self.negative_num, self.list_item_to_tensor(negative_sample))
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.load_model_pattern in ["moco", "roberta_moco"]:
            positive_sample = self.generate_single_sample(inputs["labels"])
            positive_sample = self._prepare_inputs(positive_sample)
        # if self.args.load_model_pattern not in ["moco", "roberta_moco", "original_model", "knn_bert", "scl_model", "scl_model_roberta", "knn_init_by_bert"]:
        #     negative_sample = self.generate_negative_sample(inputs["labels"])
        #     negative_sample = self._prepare_inputs(negative_sample)
        ...
        if self.use_amp:
            with autocast():
                loss = self.compute_loss(model, inputs, positive_sample, negative_sample)
        else:
            loss = self.compute_loss(model, inputs, positive_sample, negative_sample)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if self.args.load_model_pattern in ["knn_bert", "knn_roberta", "moco", "roberta_moco"]:
            # if self.args.load_model_pattern in ["knn_bert", "knn_roberta"]:
            print("******************")
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch):
        ...
        if self.control.should_evaluate:
            ...
            metrics_key.remove("epoch")
            if "eval_loss" in metrics_key:
                metrics_key.remove("eval_loss")
            self.best_metric = self.best_metric if self.best_metric is not None else metrics[metrics_key[0]]
            # fitlog.add_metric({"dev": metrics.pop("epoch")}, step=self.state.global_step)

            is_best_metric = False
            for key in metrics_key:
                metrics_value = metrics[key]
                fitlog.add_metric({key: metrics_value}, step=self.state.global_step)
                if metrics_value > self.best_metric:
                    fitlog.add_best_metric({"dev": metrics})
                    self.best_metric = metrics_value
                    is_best_metric = True

            if is_best_metric and self.test_dataset is not None:
                test_metric = self.evaluate(eval_dataset=self.test_dataset)
                fitlog.add_best_metric({"test": test_metric})
    ...
    def compute_loss(self, model, inputs, positive_sample, negative_sample):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.

        Subclass and override for custom behavior.
        """
        if self.args.load_model_pattern in ["original_model", "scl_model", "scl_model_roberta"]:
            outputs = model(**inputs)
        else:
            outputs = model(inputs, positive_sample, negative_sample)
    ...
    def evaluate(
        ...
        The calling script_v0 will be responsible for providing a method to compute metrics, as they are task-dependent
        ...
        if self.args.load_model_pattern in ["original_model", "moco", "roberta_moco", "scl_model", "scl_model_roberta",
                                            "cluster_bert"]:
    ...
    def predict(
        ...
        return self.predict_queue_knn_test(
    ...
    def prediction_loop(
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            metrics = self.compute_metrics(MyEvalPrediction(prediction_by_knn=preds, prediction_by_cls=preds,
                                                            prediction_combine=preds, label_ids=label_ids))
        else:
            metrics = {}
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if self.use_amp:
                with autocast():
                    outputs = model(**inputs)
            elif self.args.load_model_pattern in ["original_model", "scl_model", "scl_model_roberta"]:
                outputs = model(**inputs)
            elif self.args.load_model_pattern in ["moco", "roberta_moco"]:
