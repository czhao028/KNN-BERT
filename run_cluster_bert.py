...
from model import ClusterMoCoKnnBert
...
class FitLogArguments:
    ...
    num_cls_labels: int = field(default=2)
...
def main():
    ...
    config.num_cls_labels = fitlog_args.num_cls_labels
    config.top_k = training_args.top_k
    config.end_k = training_args.end_k
    config.cluster_labels = 1000
    tokenizer = AutoTokenizer.from_pretrained(
        model_args.model_name_or_path
    )

    if training_args.load_trained_model:
        model = ClusterMoCoKnnBert(config=config)
